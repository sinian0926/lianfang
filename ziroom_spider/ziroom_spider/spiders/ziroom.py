...
from ziroom_spider.items import ZufangSpiderItem as ziroom_items
...
class ZiroomSpider(scrapy.Spider):
    ...
    def page_parse(self, response):
        ...
        # # 加载ItemLoader对象
        # item_loader = ItemLoader(item=ziroom_items(), response=response)
        ...
        # 限时优惠
        label = right.xpath("./ul[@class='Z_activity']/li[1]/span[@class='label']/text()").extract()[0]
        tip = ''
        credit_plan = ''
        plan1 = ''
        plan2 = ''
        gift_plan = ''
        if '限时优惠'.__eq__(label):
            tip = right.xpath("./ul[@class='Z_activity']/li[1]/text()").extract()
            credit_plan = right.xpath("./ul[@class='Z_activity']/li[2]/text()").extract()
            plan1 = right.xpath("./ul[@class='Z_activity']/li[3]/text()").extract()
            plan2 = right.xpath("./ul[@class='Z_activity']/li[4]/text()").extract()
            gift_plan = right.xpath("./ul[@class='Z_activity']/li[5]/text()").extract()
        else:
        ...
        # 空气检测
        air = response.xpath(
            "//section[@class='Z_container Z_main']/section[@class='Z_info_main']/div[@id='areacheck']")
        # 空置时长
        air_time = air.xpath(
            "./div[@class='Z_info_body']/ul[@class='jiance']/li[1]/span[@class='info_value']/text()").extract_first()
        items["air_time"] = air_time
        # 检测日期
        air_date = air.xpath(
            "./div[@class='Z_info_body']/ul[@class='jiance']/li[2]/span[@class='info_value']/text()").extract_first()
        items["air_date"] = air_date
        # 检测报告
        air_url = air.xpath(
            "./div[@class='Z_info_body']/ul[@class='jiance']/li[3]/a[@class='info_value_active text_underline']/@href").extract_first()
        ...
        community = response.xpath(
            "//div[@id='villageinfo']/div[@class='Z_info_body']/div[@class='Z_village clearfix']/div[@class='Z_village_info']")
        # 名称
        jname = community.xpath("./h3/text()").extract_first()
        items["jname"] = jname
        # 建筑年代
        jtime = community.xpath("./ul[@class='Z_village_info_body']/li[1]/span[@class='value']/text()").extract_first()
        items["jtime"] = jtime
        # 建筑类型
        jtype = community.xpath("./ul[@class='Z_village_info_body']/li[2]/span[@class='value']/text()").extract_first()
        items["jtype"] = jtype
        # 供暖方式
        jheating = community.xpath(
            "./ul[@class='Z_village_info_body']/li[3]/span[@class='value']/text()").extract_first()
        items["jheating"] = jheating
        # 绿化率
        jgreen = community.xpath("./ul[@class='Z_village_info_body']/li[4]/span[@class='value']/text()").extract_first()
        items["jgreen"] = jgreen
        # 容积率
        plot_ratio = community.xpath(
            "./ul[@class='Z_village_info_body']/li[5]/span[@class='value']/text()").extract_first()
        items["plot_ratio"] = plot_ratio
        # 物业公司
        property = community.xpath(
            "./ul[@class='Z_village_info_body']/li[6]/span[@class='value']/text()").extract_first()
        items["property"] = property
        # 物业电话
        property_ph = community.xpath(
            "./ul[@class='Z_village_info_body']/li[7]/span[@class='value']/text()").extract_first()
        ...
        # # 抓取时间
        # item_loader.add_value("create_time", datetime.datetime.now())
        # # 修改时间
        # item_loader.add_value("modify_time", datetime.datetime.now())
        #
        # items = item_loader.load_item()

        # 抓取时间
        items["create_time"] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
        # 修改时间
        items["modify_time"] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
